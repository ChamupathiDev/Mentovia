...
const Navbar = ({ user }) => {
    ...
    const fetchUnreadCounts = async () => {
      ...
      try {
        ...
        const notifResponse = await fetch(
          `${API_BASE_URL}/users/notifications/count`,
          {
            headers: {
  ...
  const markAsRead = async (notificationId) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/users/notifications/mark-read`,
        {
          method: "POST",
          headers: {
  ...
  const clearAllNotifications = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${API_BASE_URL}/users/notifications/clear-all`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleFollowUser = async (userId, isFollowing) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/users/${endpoint}/${userId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
                        ...
                        >
                          <div className="flex items-center justify-between p-3 hover:bg-gray-50">
                            ...
                            <button
                              ...
                              onClick={() =>
                                handleFollowUser(result.id, result.isFollowing)
                              }
                            >
          ...
          <div className="flex items-center space-x-4">
            ...
            {role === 'ADMIN' && (
              <button
                className="px-3 py-1 bg-DarkColor text-white rounded-lg hover:bg-ExtraDarkColor"
